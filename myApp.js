...
const shemaPerson = new Schema({
  name: {type: String, required: true},
  age: Number,
  favoriteFoods: [String]
});
var Person = mongoose.model('Person', shemaPerson);
...
var createAndSavePerson = function(done){
  var person = new Person({name:'Jasnel Juillet', age: 23, favoriteFoods: ["vodka", "air"]});
  person.save(function(err, data) {
...
var arrayOfPeople = [
  {name: "Juillet Phalineda", age: 10, favoriteFoods: ["Del Taco"]},
  {name: "Juillet Saul", age: 30, favoriteFoods: ["Del Taco"]},
  {name: "Vilsaint Ebrans", age: 38, favoriteFoods: ["Del Taco"]},
  {name: "Emilca Edmoundo", age: 24, favoriteFoods: ["Del Taco"]}
];
...
// Person.find({favoriteFoods: foodToSearch}).Person.sort({name: -1}).Person.limit(2).Person.select({age: 0}).exec( function(err,data){
//     if(err) return console.error(err);
//     done(null,data);
//   });
/** **Well Done !!**
